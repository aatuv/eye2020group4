...
def calculateMFL_MSA(filename, subject_name, subject_results):
    with open(f'{filename}.csv', mode="a", newline='') as calc_data:
        ...
        sa_total_f = [] # saccade amplitude summed total for known=false
        fd_total_f = [] # fixation duration summed total for known=false
            ...
            for fd in sample_result[2]:
                if known == 'true':
                    ...
                    if j > 0:
                        last_cntr = sample_result[1][j - 1]
                        cur_cntr = sample_result[1][j]
                        sa_total_f.append(math.hypot(cur_cntr[0], last_cntr[0], cur_cntr[1], last_cntr[1]))
                        sa_count_f += 1
                j += 1
            i += 1
